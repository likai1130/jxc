            ...
            <el-card class="box-card">
                <el-form ref="queryForm" :model="queryParams" :inline="true" label-width="68px">
                    <el-form-item label="商品名称" prop="gName"><el-input v-model="queryParams.gName" placeholder="请输入商品名称" clearable
                        ...
                        <el-form-item label="采购单" prop="purchaseListId"><el-input v-model="queryParams.purchaseListId" placeholder="请输入采购单" clearable
                                              size="small" @keyup.enter.native="handleQuery"/>
                            </el-form-item>
                ...
                <el-row :gutter="10" class="mb8">
                    <el-col :span="1.5">
                        <el-button
                                v-permisaction="['jxc:purchaseListGoods:add']"
                                type="primary"
                                icon="el-icon-plus"
                                size="mini"
                                @click="handleAdd"
                    ...
                    <el-col :span="1.5">
                        <el-button
                                v-permisaction="['jxc:purchaseListGoods:edit']"
                                type="success"
                                icon="el-icon-edit"
                                size="mini"
                                :disabled="single"
                    ...
                    <el-col :span="1.5">
                        <el-button
                                v-permisaction="['jxc:purchaseListGoods:remove']"
                                type="danger"
                                icon="el-icon-delete"
                                size="mini"
                                :disabled="multiple"
                ...
                <el-table v-loading="loading" :data="purchaseListGoodsList" @selection-change="handleSelectionChange">
                    <el-table-column type="selection" width="55" align="center"/><el-table-column label="商品编码" align="center" prop="gCode"
                                                 ...
                                                 :show-overflow-tooltip="true"/><el-table-column label="采购单" align="center" prop="purchaseListId"
                                                 :show-overflow-tooltip="true"/><el-table-column label="商品" align="center" prop="goodsId"
                                                 :show-overflow-tooltip="true"/><el-table-column label="供货商" align="center" prop="supplierId" :formatter="supplierIdFormat" width="100">
                                <template slot-scope="scope">
                                    {{ supplierIdFormat(scope.row) }}
                                </template>
                            </el-table-column>
                ...
                <el-dialog :title="title" :visible.sync="open" width="500px">
                    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                                    ...
                                    <el-form-item label="总价" prop="totalPrice">
                                        <el-input v-model="form.totalPrice" placeholder="总价"
                                                      />
                                    </el-form-item>
                                    <el-form-item label="采购单" prop="purchaseListId">
                                        <el-input v-model="form.purchaseListId" placeholder="采购单"
                                                      />
                                    </el-form-item>
                                    <el-form-item label="商品" prop="goodsId">
                                        <el-input v-model="form.goodsId" placeholder="商品"
                                                      />
                                    </el-form-item>
...
<script>
    import {addPurchaseListGoods, delPurchaseListGoods, getPurchaseListGoods, listPurchaseListGoods, updatePurchaseListGoods} from '@/api/jxc/tb-purchase-list-goods'
    
    import {listSupplier } from '@/api/jxc/tb-supplier'
    export default {
        name: 'PurchaseListGoods',
        components: {
        },
        data() {
            return {
                ...
                // 表单校验
                rules: {gName:  [ {required: true, message: '商品名称不能为空', trigger: 'blur'} ],
                purchaseListId:  [ {required: true, message: '采购单不能为空', trigger: 'blur'} ],
                }
        }
        },
        created() {
        ...
        methods: {
            ...
            getList() {
                this.loading = true
                listPurchaseListGoods(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
            ...
            handleUpdate(row) {
                ...
                const id =
                row.id || this.ids
                getPurchaseListGoods(id).then(response => {
                ...
                this.$refs['form'].validate(valid => {
                    if (valid) {
                        if (this.form.id !== undefined) {
                            updatePurchaseListGoods(this.form).then(response => {
                            ...
                            addPurchaseListGoods(this.form).then(response => {
                                if (response.code === 200) {
                                    this.msgSuccess(response.msg)
                                    this.open = false
                                    this.getList()
            ...
            handleDelete(row) {
                ...
                this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
                    ...
                    type: 'warning'
                      ...
                      return delPurchaseListGoods( { 'ids': Ids })
                }).then((response) => {
                   if (response.code === 200) {
                     this.msgSuccess(response.msg)
                     this.open = false
